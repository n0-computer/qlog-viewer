...
impl PacketizationDiagram {
    ...
    fn render_byte_stream(
        ...
        ui.allocate_ui(Vec2::new(ui.available_width(), scroll_area_height), |ui| {
            ui.horizontal(|ui| {
                // Left: Labels column (fixed 90px)
                let label_rect = ui.allocate_space(Vec2::new(90.0, content_height)).1;
                let labels = ["Stream IDs", "Stream Data", "QUIC Frames", "QUIC Packets"];
                for (i, label) in labels.iter().enumerate() {
                    let y = label_rect.top() + 5.0 + (i as f32 * LANE_HEIGHT) + LANE_HEIGHT / 2.0;
                    ui.painter().text(
                        Pos2::new(label_rect.left() + 5.0, y),
